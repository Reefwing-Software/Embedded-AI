...
void loop() {
  ...
  //  Compensate for tilt
  float mag_x_compensated = imu.data.mx * cos(pitch) 
                        - imu.data.my * sin(pitch) * sin(roll) 
                        + imu.data.mz * sin(pitch) * cos(roll);
  float mag_y_compensated = imu.data.my * cos(roll) 
                          + imu.data.mz * sin(roll);
