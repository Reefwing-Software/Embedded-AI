...
def create_dct_measurement_matrix(num_samples, indices):
    # Create a DCT matrix (Type II)
    dct_matrix = dct(np.eye(num_samples), norm='ortho')
    
    # Select random rows based on the provided indices
    measurement_matrix = dct_matrix[indices, :]
    
    return measurement_matrix

# Reconstruct signal using L1 minimization with DCT basis
def reconstruct_signal(num_samples, compressed_samples):
    ...
    # Create the measurement matrix using DCT-based sampling
    indices = list(compressed_samples.keys())
    Phi = create_dct_measurement_matrix(num_samples, indices)
    
    # L1 minimization problem using CVXPY
    x = cp.Variable(num_samples)  # Reconstructed signal
    objective = cp.Minimize(cp.norm1(x))  # L1 norm minimization
    
    # Constraints: Ensure the known measurements match, with relaxed tolerance
    constraints = [cp.norm(Phi @ x - y_normalized, 2) <= 1e-3]
    ...
    # Return the reconstructed signal only if the solver found an optimal solution
    if problem.status == 'optimal':
        reconstructed_signal_normalized = x.value
        ...
        print("Problem was not solved optimally.")
        return None
