...
# Reconstruct signal using B-spline
def reconstruct_signal(num_samples, compressed_samples):
    # Extract and sort the indices and values from compressed_samples
    compressed_sample_indices = np.array(list(compressed_samples.keys()))
    compressed_sample_values = np.array(list(compressed_samples.values()))

    # Sort the indices and corresponding values
    sorted_indices = np.argsort(compressed_sample_indices)
    compressed_sample_indices = compressed_sample_indices[sorted_indices]
    compressed_sample_values = compressed_sample_values[sorted_indices]

    # Create a B-Spline basis for interpolation
    spline = make_interp_spline(compressed_sample_indices, compressed_sample_values, k=0)

    # Interpolate the full signal using the spline
    time_full = np.arange(num_samples) 
    reconstructed_signal = spline(time_full)
