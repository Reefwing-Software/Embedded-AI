...
#define MAX_ARP_NOTES 16       // Maximum notes in an arpeggio sequence
...
void midi_send(uint8_t note, uint8_t velocity, bool note_on) {
    printf("MIDI %s - Note: %d, Velocity: %d\n", note_on ? "Note On" : "Note Off", note, velocity);
}
...
void midi_sequence(const uint8_t* note_sequence, size_t sequence_size) {
  ...
  if (current_ms - start_ms < 286) return; // Not enough time
  start_ms += 286;
  ...
  if (note_pos >= sequence_size) {
    ...
    note_pos = 0;
    printf("Arpeggio sequence finished\n");
  } else {
...
void led_blinking_task(void) {
  ...
  if (current_ms - start_ms < blink_interval_ms) return; // Not enough time
  start_ms = current_ms; // Update the timestamp
      ...
      for (int i = 0; i < TOUCH_NUMBER; i++) {
          if (new_keys_pressed & (1 << i)) {
              if (i == 11) {          // GP13: "Arpeggio" button
                printf("Arpeggio button pressed\n");
                arpeggio_size = generate_arpeggio(48, CHORD_MAJOR, 2, arpeggio_sequence, MAX_ARP_NOTES);
                printf("Generated arpeggio with %d notes\n", arpeggio_size);
                if (arpeggio_size > 0) {
...
int generate_arpeggio(uint8_t root_note, ChordType chord_type, uint8_t num_cycles, uint8_t* note_sequence, size_t max_size) {
    ...
    printf("Generated arpeggio sequence: ");
    for (int i = 0; i < index; i++) {
        printf("%d ", note_sequence[i]);
    }
    printf("\n");
